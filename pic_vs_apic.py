    ...
    for p in x:
        ...
        for i in ti.static(range(3)):
            for j in ti.static(range(3)):
                ...
                weight = w[i][0] * w[j][1]
                grid_v[base + offset] += weight * (v[p] + affine @ dpos)  # new   P2G
                grid_m[base + offset] += weight
    ...
    for p in x:
        ...
        for i in ti.static(range(3)):
            for j in ti.static(range(3)):
                ...
                # weight_sum += weight
                new_v += weight * g_v
                new_C += 4 * weight * g_v.outer_product(dpos) * inv_dx  # new 
        # print(weight_sum)
...
def reset(mode: ti.i32):
    for i in range(n_particles):
        ...
        if mode == 0:
            ...
            v[i] = [x[i][1] - 0.5, 0.5 - x[i][0]]  # rotation at (0.5,0.5)
        elif mode == 2:
            v[i] = [0, x[i][0] - 0.5]   # shearing    rotation 其实际上两个方向上的shearing?
        else:
            v[i] = [0, x[i][1] - 0.5]  # y axis dilation
        
reset(1)

gui = ti.GUI("PIC v.s. APIC", (750, 750))
